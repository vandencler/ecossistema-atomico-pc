...
### 3.1. Indexing
```sql
-- Call Code Optimization
CREATE INDEX IF NOT EXISTS idx_pessoas_cdchamada_trgm 
ON wshop.pessoas USING gin (cdchamada gin_trgm_ops);

-- Split Phone Optimization
...
The application is failing with `permission denied` on multiple lookup tables.

```sql
-- Ensure both roles have usage on the schema
GRANT USAGE ON SCHEMA wshop TO eav_writer, eav_reader;

-- Grant SELECT on all required lookup tables
GRANT SELECT ON wshop.pessoas TO eav_writer, eav_reader;
GRANT SELECT ON wshop.crediar TO eav_writer, eav_reader;
GRANT SELECT ON wshop.tabelaprecos TO eav_writer, eav_reader;
GRANT SELECT ON wshop.documen TO eav_writer, eav_reader;
GRANT SELECT ON wshop.docitem TO eav_writer, eav_reader;
GRANT SELECT ON wshop.produto TO eav_writer, eav_reader;
GRANT SELECT ON wshop.grupo TO eav_writer, eav_reader;
GRANT SELECT ON wshop.pessoas_endereco TO eav_writer, eav_reader;
GRANT SELECT ON wshop.documento_nfce TO eav_writer, eav_reader;

-- Grant UPDATE on sync targets (SAV Workflow)
...
- `\d wshop.pessoas` should show the new trigram indexes.
